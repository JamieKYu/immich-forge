# -*- coding: utf-8 -*-
"""SCUNet (Swin-Conv-UNet) architecture for blind real-image denoising.

Vendored verbatim from the official implementation by Kai Zhang
(https://github.com/cszn/SCUNet, Apache-2.0 — see LICENSE in this directory),
with three local edits so it stands alone in this repo:

  * the `thop` profiling import and the `__main__` smoke test are dropped;
  * the per-block `print(...)` is removed (it spammed stdout on every build);
  * `trunc_normal_` and `DropPath` are inlined below instead of imported from
    `timm`, so this module needs only torch + einops (no timm — matching the
    dependency-light vendoring of ddcolor/).

`einops` is the only non-torch dependency and is pulled in via
requirements-ml.txt. The pretrained colour denoising checkpoints
(scunet_color_real_psnr / _gan) use config=[4,4,4,4,4,4,4], dim=64.
"""
import warnings
...
# --- inlined from timm.models.layers (Apache-2.0) so timm isn't a dependency ---
